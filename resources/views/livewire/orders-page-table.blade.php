            ...
            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700 border-b">
                ...
                <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap dark:text-white">
                    @if (!$order->is_processed)
                        <span class="px-3 py-1 rounded-xl bg-blue-500 text-blue-100">Pending...</span>
                    @endif
                </td>
                <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap dark:text-white">
                    <button wire:click="changeOrderStatus({{ $order->id }})" class="text-gray-600 font-semibold text-sm bg-gray-200 px-3 py-2 border border-gray-300 rounded mr-2">Complete Order</button>
                </td>
            </tr>
            @empty
            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700 border-b">
                <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap dark:text-white text-center" colspan="10">No new orders</td>
            </tr>
            @endforelse
        </table>
    </div>
